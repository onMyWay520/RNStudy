                    ...
                    // this.state.isShowBottom == true ?
                        <View style={{width: width, height: 49, backgroundColor:"#FFF",
                            ...
                            <TouchableOpacity style={{flexDirection:'row'}} onPress={() => this.allSelect(isChecked)}>
                                <Image style={{marginLeft:10, width:30,height:30}} source={isChecked ? require("../image/icon_check.png") :
                                    require("../image/icon_no_check.png")}/>
                                <Text style={{marginLeft:10, color: "#cc3341", marginTop:10}}>全选</Text>
                            </TouchableOpacity>
    ...
    renderRow = (rowData, sectionID, rowID) => { // cell样式
        ...
        return (
            <View style={{width: width, height: 60, backgroundColor:"#EEEEDD",
                ...
                marginBottom: 10, marginLeft: this.state.isEdit ? 40 : 0}}>
                {
                    // this.state.isEdit ?
                        <TouchableOpacity style={{positionvfdcfdxcd:'absolute'}} onPress={
                            () => this.selectItem(parseInt(rowID), rowData.collectItem ,isChecked)}>
                            <Image   source={isChecked ? require("../image/icon_check.png") :
                                require("../image/icon_no_check.png")}
                                     style={{width: 25, height: 25,left:20,alignItems:'center'}}/>
                        </TouchableOpacity>
                    // : null
                }
                <Text style={{fontSize:18,color:'red',left:60}}>{rowData.collectItem}</Text>
            </View>
        )
    }
